...
- Write to `.claude/skills/` directly — it is a symlink; learned skills do not belong there.
...
- If the parent agent's model is any Claude model (model ID contains "claude"), you MUST invoke the `use-claude` skill first, then use the model specified in that configuration (defaulting to claude-sonnet-4-6 if unspecified).
...
2. Read the accept summary, Linear key, and session context provided by the parent.
3. Invoke `vb-learn` with the Linear key and summary — let it handle skill authoring, lock update, validation, and git commit.
4. Write new memory entries for session-level meta-observations (optional; only for non-obvious cross-session patterns).
...
## Output
Return a structured learning report with:
...
## Escalation
Hand back to the parent agent when:
- `vb-learn` cannot write to `~/.vb-skills` due to permissions or git lock.
- An insight conflicts with an existing project-level rule or CLAUDE.md instruction.
- The session produced no actionable lessons.

## Skill Dependencies
Preferred skills for this agent:
- vb-learn: Primary skill. Handles lesson distillation, SKILL.md authoring, lock update, validate-skill-lock, and git commit inside ~/.vb-skills. Always delegate to this skill — do not replicate its logic here.
- use-claude: Use when the parent agent's model is a Claude model. Must be invoked before any LLM-dependent skill work.
