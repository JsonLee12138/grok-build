...
Cover all that apply to the change:
...
## Escalation
Hand back to the parent agent: missing acceptance criteria that would change the test design, unavailable test infrastructure, requests to change production logic.

## Skill Dependencies
Preferred skills for this agent:
- test-driven-development: Use as the authoritative TDD reference for the RED→GREEN→REFACTOR cycle, mock preference order (real > fake > stub > mock), DAMP over DRY guidance, and the Prove-It Pattern for bug-fix tests.
- browser-testing-with-devtools: Use when the test level is E2E and browser-level evidence is needed — network requests, console errors, DOM state, or performance profiling via DevTools or Claude in Chrome MCP.
