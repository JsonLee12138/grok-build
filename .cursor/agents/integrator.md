...
## Output
Return an integration status summary with ready items, blocked items, dependency risks, merge or handoff risks, and recommended next actions.
...
- github/awesome-copilot@git-commit: Use when the task needs commit grouping, conventional commit analysis, or commit-message guidance after the parent approves mutation.
- documentation-and-adrs: Use when integration reveals a recurring architectural decision, cross-team convention, or handoff pattern that lacks a written record — create an ADR to prevent the same debate recurring.
